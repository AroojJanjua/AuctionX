...
<body>
   ...
   <nav class="navbar navbar-expand-lg ax-navbar sticky-top">
    <div class="container">
      ...
      <img src="{{ asset('image/auctionxlogo.png') }}" alt="AuctionX" class="navbar-logo-img"></a>
      ...
      <li class="nav-item">
          ...
          <i class="bi bi-grid me-1"></i>View Items</a>
        </li>
        @auth
          @if(auth()->user()->role === 'seller')
          <li class="nav-item">
            <a class="nav-link {{ request()->routeIs('seller.*') ? 'active' : '' }}" href="{{ route('seller.dashboard') }}">
              <i class="bi bi-tag me-1"></i>Sell</a>
          </li>
          @endif
        @endauth
        <li class="nav-item">
          <a class="nav-link {{ request()->routeIs('how-it-works') ? 'active' : '' }}"
            href="{{ route('how-it-works') }}"><i class="bi bi-info-circle me-1"></i>How It Works</a>
        </li>
      </ul>
    ...
    class="ax-search-form d-flex mx-auto mt-2 mt-lg-0">
      <input type="text" name="search" class="form-control" placeholder="search about auction..."
      ...
      @else
        ...
        <div class="dropdown" id="notifDropdown">
          <button
            ...
            aria-expanded="false" title="Notifications" onclick="loadNotifDropdown()">
          ...
          <div class="dropdown-menu dropdown-menu-end p-0 shadow" style="width:320px;border-radius:14px;
          border:1px solid var(--border);font-family:'Nunito',sans-serif;overflow:hidden">
            {{-- Header --}}
        ...
        <div class="dropdown">
          <button class="btn btn-ghost-ax btn-sm px-3 dropdown-toggle" data-bs-toggle="dropdown">
            ...
            @if(in_array(auth()->user()->role,['bidder']))
            <li><a class="dropdown-item"  href="{{ route('my-bids') }}">
                <i class="bi bi-coin me-2"></i>My Bids</a></li>
            @endif

            @if(in_array(auth()->user()->role,['seller']))
            <li><a class="dropdown-item {{ request()->routeIs('seller.earnings') }}"
            href="{{ route('seller.earnings') }}"><i class="bi bi-cash me-2"></i>My Earnings</a></li>
            @endif
            ...
            <li><a class="dropdown-item {{ request()->routeIs('admin.messages.*') ? 'active' : '' }}"
            href="{{ route('admin.messages.index') }}"><i class="bi bi-envelope me-2"></i>Contact Messages</a>
            <li><a class="dropdown-item" href="{{ route('admin.reports') }}">
            <i class="bi bi-bar-chart me-2"></i>Reports</a></li>
            @endif
  ...
  <footer class="ax-footer py-4 mt-auto">
    ...
    <div class="col-md-8">
      <div class="d-flex flex-wrap justify-content-md-center gap-3">
        ...
        <a href="{{ route('auctions.index') }}" class="footer-link">Auctions</a>
