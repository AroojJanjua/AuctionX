    ...
    <div class="section-title mb-4" style="font-size:1rem">
      ...
      <form method="POST" action="{{ route('profile.update') }}">
          ...
          <div class="mb-3">
            ...
            <input type="text" name="name" class="form-control-ax" value="{{ old('name', $user->name) }}" required />
            @error('name')<div style="font-size:.78rem;color:var(--red);margin-top:4px">{{ $message }}</div>@enderror
          </div>

          <div class="mb-3">
            <label class="form-label-ax">Email Address</label>
            <input type="email" name="email" class="form-control-ax" value="{{ old('email', $user->email) }}" required />
            @error('email')<div style="font-size:.78rem;color:var(--red);margin-top:4px">{{ $message }}</div>@enderror
          </div>
      ...
      <div style="background:#fff;border:1px solid var(--border);border-radius:16px;padding:1.8rem;margin-bottom:1.2rem">
        ...
        <form method="POST" action="{{ route('profile.update') }}">
          ...
          <div class="mb-3">
            ...
            <select name="payout_method" class="form-control-ax">
            ...
            @error('payout_method')<div style="font-size:.78rem;color:var(--red);margin-top:4px">{{ $message }}</div>@enderror
          </div>
    
          <div class="mb-3">
            <label class="form-label-ax">Account Number</label>
            <input type="text" name="payout_account_number" class="form-control-ax"
            value="{{ old('payout_account_number', $user->payout_account_number) }}" placeholder="Enter account number" />
            @error('payout_account_number')<div style="font-size:.78rem;color:var(--red);margin-top:4px">{{ $message }}</div>@enderror
          </div>
    
          <div class="mb-4">
            <label class="form-label-ax">Account Holder Name</label>
            <input type="text" name="payout_account_name" class="form-control-ax"
                   value="{{ old('payout_account_name', $user->payout_account_name) }}" placeholder="Name as registered on the account" />
            @error('payout_account_name')<div style="font-size:.78rem;color:var(--red);margin-top:4px">{{ $message }}</div>@enderror
          </div>
          <button type="submit" class="btn btn-brown-outline px-5">Save</button>
        </form>
      </div>
      ...
      <div class="section-title mb-4" style="font-size:1rem">
        ...
        <form method="POST" action="{{ route('profile.password') }}">
          ...
          <div class="mb-3">
            ...
            <input type="password" name="current_password" class="form-control-ax" placeholder="Enter current password" />
            @error('current_password')<div style="font-size:.78rem;color:var(--red);margin-top:4px">{{ $message }}</div>@enderror
          </div>

          <div class="mb-3">
            <label class="form-label-ax">New Password</label>
            <input type="password" name="password" class="form-control-ax" placeholder="Min 8 characters" />
          </div>

          <div class="mb-4">
            <label class="form-label-ax">Confirm New Password</label>
            <input type="password" name="password_confirmation" class="form-control-ax" placeholder="Repeat new password" />
          </div>
